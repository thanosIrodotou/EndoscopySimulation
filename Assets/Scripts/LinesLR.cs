...
public class LinesLR : MonoBehaviour {
	...
	private float lineSizeLarge = 0.01f;
	private float lineSizeSmall = 0.01f;
	public Vector3 last = new Vector3 (1.604631f, 0.5567473f, -0.5761633f);
	
	private Color lineColorLarge = new Color(0,0,0,0.5f);
	private Color lineColorSmall = new Color(0,0,0,0.1f);
	
	public GameObject endoscopeObject;
	public Transform endoscopeTransform;
...
//		}
		...
		//cubeTransform.transform.position += new Vector3(-0.01f,0.01f,0);
		//endoscopeTransform.Translate(Time.deltaTime*5f,0,0);
		//endoscopeTransform.Rotate(0,0,Time.deltaTime*50f);
		if (Input.GetKey(KeyCode.E)) {	
			
			curr = new Vector3(GameObject.Find("First Person Controller").transform.position.x, 0.5567473f, GameObject.Find("First Person Controller").transform.position.z);			
			//curr = transform.InverseTransformPoint(curr);
			print (last + ", " + curr);
			//print (last + ", " + curr);
//			points.Add(GameObject.Find("Cube").transform.position);
//			points.Add(GameObject.Find("Sphere").transform.position);
//			if(last.z != -100.0f) 
//			{
...
//		print (cubeObject.transform.position + ", " + curr);
